...
echo [BUILD] Windows Clang - ServerEngine+Projects Release

call "%~dp0..\..\_setup_windows.bat"
if %ERRORLEVEL% neq 0 ( pause & exit /b 1 )

for %%I in ("%~dp0..\..\..") do set "ROOT=%%~fI"
set "BD=%ROOT%\build\windows-clang\Release"
...
"%CMAKE%" --build "%BD%" --parallel --target ServerEngine TestServer DummyClient ServerMonitor
if %ERRORLEVEL% neq 0 ( echo [ERROR] Build failed. & pause & exit /b %ERRORLEVEL% )

echo [SUCCESS] ServerEngine+Projects Release complete.
pause
endlocal