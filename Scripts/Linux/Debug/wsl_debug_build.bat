...
echo [BUILD] Linux Debug via WSL2
...
wsl bash "%WSL_ROOT%/Scripts/Linux/_wsl_build.sh" Debug "%WSL_ROOT%"
if %ERRORLEVEL% neq 0 ( echo [ERROR] Linux Debug build failed. & pause & exit /b %ERRORLEVEL% )

echo [SUCCESS] Linux Debug binaries: bin\linux\debug\
pause
endlocal